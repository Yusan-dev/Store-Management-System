...
/* ---------- EXTRACT ARTIKEL (11 char) ---------- */
function extractArtikel11(variant) {
  if (!variant || variant.length < 11) return variant || "";
  return variant.substring(0, 11);
}
...
function extractSize(variant) {
  ...
  // Remove the first 11 chars (artikel), look at the rest
  const suffix = variant.length > 11 ? variant.substring(11) : variant;
...
function kangodingPrice(price, category) {
  ...
  if (category === "FOOTWEAR" && price <= 200000 && price % 1000 === 0) return "FREEFALL";
  if (category !== "FOOTWEAR" && price < 25000 && price % 1000 === 0) return "FREEFALL";
...
function processBrokenSize(rows) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const artikel = extractArtikel11(variant);
    if (!artikel) continue;
...
function drawBsTable(rows) {
  ...
  rows.forEach(r => {
    const statusClass = r.status === "BROKEN" ? "style=\"color:#FF0000;font-weight:800\"" 
                       : r.status === "NOT BROKEN" ? "style=\"color:#00AA00;font-weight:800\"" 
                       : r.status === "UNCATEGORIZE" ? "style=\"color:#888888;font-weight:800\""
                       : "";
...
function updateBsSummary(rows) {
  ...
  document.getElementById("bsBrokenPct").innerText = total ? (broken / total * 100).toFixed(1) + "%" : "0%";
  
  document.getElementById("bsNotBrokenCount").innerText = notBroken.toLocaleString();
  document.getElementById("bsNotBrokenPct").innerText = total ? (notBroken / total * 100).toFixed(1) + "%" : "0%";

  // Apparel broken
  const apparelAll = rows.filter(x => x.category === "APPAREL");
  const apparelBroken = apparelAll.filter(x => x.status === "BROKEN").length;
  document.getElementById("bsApparelBroken").innerText = apparelBroken.toLocaleString();
  document.getElementById("bsApparelBrokenPct").innerText = apparelAll.length 
    ? (apparelBroken / apparelAll.length * 100).toFixed(1) + "% of " + apparelAll.length 
    : "0%";

  // Footwear broken
  const footwearAll = rows.filter(x => x.category === "FOOTWEAR");
  const footwearBroken = footwearAll.filter(x => x.status === "BROKEN").length;
  document.getElementById("bsFootwearBroken").innerText = footwearBroken.toLocaleString();
  document.getElementById("bsFootwearBrokenPct").innerText = footwearAll.length 
    ? (footwearBroken / footwearAll.length * 100).toFixed(1) + "% of " + footwearAll.length 
    : "0%";
}
