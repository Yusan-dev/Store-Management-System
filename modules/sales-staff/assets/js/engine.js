...
function isNonMD(article){
    article = text(article);
    const category = getCategory(article).toUpperCase();
    return (
        category === "PAPERBAG" ||
        category === "NON-MD" ||
...
//     }
// =====================================================
function getAllStaffDailySummary(dateFilter = "") {
    const summary = new Map();

    transactions.forEach(t => {
        if(!matchDateFilter(t.date, dateFilter)) return;
        if(t.isNonMD) return;
        if(t.isReturn) return;
        
        const tStaff = t.staff ? text(t.staff.name) : "UNKNOWN";
        const dateKey = t.date;
        const rowKey = dateKey + "_" + tStaff;
        
        if(!summary.has(rowKey)){
            summary.set(rowKey, {
                staff: tStaff,
                date: dateKey,
                sales: 0,
        ...
        const row = summary.get(rowKey);
        row.sales += t.sales;
        row.qty += t.qty;
        
        const invKey = t.invoice;
        if(!row.invoices.has(invKey)){
            row.invoices.add(invKey);
            row.sm++;
        }
        
        const division = getCategory(t.article) || "UNCATEGORIZED";
        row.categories[division] = (row.categories[division] || 0) + t.qty;
    });

    const staffSummaryArray = Array.from(summary.values());
    
    // Sort by staff name, then by Date
    staffSummaryArray.sort((a, b) => {
        if (a.staff !== b.staff) {
    ...
    staffSummaryArray.forEach(row => {
        totalSales += row.sales;
        totalQty += row.qty;
        row.invoices.forEach(inv => totalInvoices.add(inv));
        Object.keys(row.categories).forEach(cat => {
            totalCategories[cat] = (totalCategories[cat] || 0) + row.categories[cat];
        });
    });

    if(staffSummaryArray.length > 0){
        staffSummaryArray.push({
            staff: "TOTAL",
            date: "TOTAL",
            sales: totalSales,
            sm: totalInvoices.size,
            qty: totalQty,
            categories: totalCategories,
...
function getStaffDailySummary(staffName, dateFilter = "") {
    const summary = new Map();
    let selectedStaff = text(staffName);
    if(selectedStaff === "?? O2O") { selectedStaff = "O2O"; }
    ...
    transactions.forEach(t => {
        ...
        const tStaff = t.staff ? text(t.staff.name) : "UNKNOWN";
        if(tStaff !== selectedStaff) return;
