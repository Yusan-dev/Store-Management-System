...
function updateLicenseStatusUI() {
  ...
  if (userIdElement) {
    userIdElement.innerText = status.userId || "-";
  }
  ...
  if (status.plan === "VIP_LORD") {
    if (planElement) {
      planElement.innerText = "👑 VIP LORD";
    }
  ...
  if (status.plan === "VIP_LIFETIME") {
    if (planElement) {
      planElement.innerText = "◆ VIP LIFETIME";
    }
  ...
  if (status.plan === "FREE_ACCESS") {
    if (planElement) {
      planElement.innerText = "FREE ACCESS";
    }
  ...
  if (status.plan === "FREE_EXPIRED") {
    if (planElement) {
      planElement.innerText = "ACCESS EXPIRED";
    }

    if (infoElement) {
      infoElement.innerText = "VIP LIFETIME ACCESS REQUIRED";
    }
  ...
  if (planElement) {
    planElement.innerText = "NOT AUTHORIZED";
  }
...
function showExpiredAccessScreen() {
  ...
  const screen = document.getElementById("licenseAccessScreen");

  const userIdElement = document.getElementById("expiredUserId");
  ...
  if (expiredDateElement) {
    expiredDateElement.innerText = formatLicenseDate(status.freeLimit);
  }
...
function readExcel(file) {
  return new Promise((resolve, reject) => {
    if (!file) {
      reject(new Error("Excel file tidak ditemukan."));
...
function validateFiles() {
  ...
  if (!files.salesPerson) {
    alert("Upload Salesperson Wise.");

    return false;
  }

  if (!files.msr) {
    alert("Upload Merchandise Sales Report.");
...
function debugEngine(summary) {
  console.group("GT AUTO SALES ENGINE RESULT");

  console.table(summary);

  const divisions = getDynamicDivisions(summary);
  ...
  if (total) {
    const categoryQty = Object.values(total.categories || {})
...
async function runProcess() {
  ...
  try {
    ...
    console.log("===== GT AUTO SALES START =====");
    ...
    const runtimeAccess = GTRuntime.authorize(dailyRows);

    console.log("GT RUNTIME ACCESS:", runtimeAccess);
    ...
    if (!runtimeAccess.ok) {
      ...
      if (runtimeAccess.reason === "FREE_ACCESS_EXPIRED") {
    ...
    const summary = GTEngine.generateSummary();
    ...
    if (!Array.isArray(summary)) {
      throw new Error("Engine menghasilkan summary invalid.");
    }
    ...
    if (typeof configurePerformanceDateLimits === "function") {
    ...
    if (typeof updateDailyValidation === "function") {
    ...
    console.log("===== GT AUTO SALES FINISHED =====");
