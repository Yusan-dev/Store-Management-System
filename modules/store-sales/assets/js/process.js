...
function updateLicenseStatusUI(){
    ...
    if(status.plan === "VIP_LORD"){
        ...
        if(infoElement) infoElement.innerText = `${status.accountName || "KANGODING.ORG"} • ${status.authority || "SOVEREIGN"} • PERMANENT ACCESS`;
        return;
    }
    if(status.plan === "VIP_LIFETIME"){
        if(planElement) planElement.innerText = "◆ VIP LIFETIME";
        if(infoElement) infoElement.innerText = `${status.accountName || "VIP MEMBER"} • PERMANENT ACCESS • NO EXPIRATION`;
        return;
    }
    if(status.plan === "FREE_ACCESS"){
        if(planElement) planElement.innerText = "FREE ACCESS";
        if(infoElement) infoElement.innerText = "ACTIVE UNTIL • " + formatLicenseDate(status.freeLimit);
        return;
    }
    ...
    if(infoElement) infoElement.innerText = "PROCESS DAILY CASH TO VERIFY ACCESS";
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("process").onclick = async () => {
        if (!files.msr) { alert("Upload Merchandise Sales Report (MSR)"); return; }
        ...
        if (!files.target) { 
            ...
            if(saved) {
                ...
                alert("Upload Target Harian"); return; 
            }
        }
        
        if (!files.dailyCash) { alert("Upload Daily Cash Collection"); return; }
        if (!files.advOrd) { alert("Upload Salesperson / Advance Order"); return; }
        ...
        try {
            ...
            if (files.target) {
                ...
                localStorage.setItem("gt_store_target_data", JSON.stringify(targetData));
            }
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                const rows = window.XLSX.utils.sheet_to_json(workbook.Sheets[firstSheet], { header: 1 });
                resolve(rows);
            } catch (err) {
                reject(err);
            }
...
function gesttechPrice(price, division) {
    ...
    const isFW = division.includes("FOOTWEAR") || division.includes("SHOES") || division.includes("SEPATU");
...
function getCatKey(division) {
    ...
    if (d.includes("APP") || d.includes("APPAREL") || d.includes("CLOTHING")) return "APP";
    return "OTHER";
}
...
function parseAllData(msrData, targetData, dailyCashData, advOrdData, targetStore) {
    const parseRupiah = (val) => {
        if (typeof val === "number") return val;
        const str = String(val || "").trim();
        return Number(str.replace(/\./g, "").replace(/[^\d-]/g, "")) || 0;
    };
    const dataByDate = {}; let o2oInvoices = {};
    const targetMap = {};
    ...
    for(let i=0; i<msrData.length; i++) {
        ...
        // Exclude paperbag
        const rawArticle = String(row[5] || "").trim().toUpperCase();
        if(rawArticle.startsWith("ZSP") || rawArticle.startsWith("ZZZ") || rawArticle.startsWith("NON")) continue;

        const division = String(row[1] || "").trim().toUpperCase();
        if(!division || division.startsWith("TOTAL") || division.startsWith("GRAND") || division === "PRODUCT DIVISION" || division === "NON-MD" || division === "NON MD") continue;
        ...
        if(!dataByDate[date]) {
            dataByDate[date] = { 
                sales: 0, qty: 0, sm: 0, 
                acc: 0, accSales: 0, bags: 0, bagsSales: 0, app: 0, appSales: 0, ftw: 0, ftwSales: 0, o2oSales: 0, o2oSM: 0, o2oQty: 0,
                targetPercent: 0,
                dayOfMonth: parseInt(date.split("-")[0], 10),
                articles: {},
                catSM: { ACC: new Set(), BAG: new Set(), APP: new Set(), FTW: new Set() }
            };
        }
        ...
        if (division.includes("ACC") || division.includes("ACCESSORIES")) { dataByDate[date].acc += qty; dataByDate[date].accSales += netAmt; }
        if (division.includes("BAG") || division.includes("TAS")) { dataByDate[date].bags += qty; dataByDate[date].bagsSales += netAmt; }
        if (division.includes("SHOES") || division.includes("FOOTWEAR")) { dataByDate[date].ftw += qty; dataByDate[date].ftwSales += netAmt; }
        else if (division.includes("APP") || division.includes("APPAREL") || division.includes("CLOTHING")) { dataByDate[date].app += qty; dataByDate[date].appSales += netAmt; }
        ...
        if (qty > 0) {
            ...
            if (!dataByDate[date].discountData[catKey]) dataByDate[date].discountData[catKey] = {};
        ...
        if (article) {
            if (!dataByDate[date].articles[article]) {
                dataByDate[date].articles[article] = { qty: 0, sales: 0, division: division };
            }
    ...
    for (let i = 0; i < advOrdData.length; i++) {
        ...
        if (row.length === 2 && typeof row[1] === "string" && /[A-Z0-9]{5,}/.test(row[1])) {
        ...
        if (currentAdvDate && dataByDate[currentAdvDate] && currentArticle) {
            ...
            if (artData) {
                ...
                if (dataByDate[currentAdvDate].catSM && dataByDate[currentAdvDate].catSM[catKey]) {
...
function buildDateDropdowns() {
    const dates = Object.keys(window.storeData.dates).sort((a,b) => dateToComparable(a) - dateToComparable(b)); // Ascending
    ...
    if(selFrom) {
        ...
        dates.forEach(d => {
            const opt = document.createElement("option"); opt.value = d; opt.innerText = d; selFrom.appendChild(opt);
        });
    ...
    if(selTo) {
        ...
        dates.forEach(d => {
            const opt = document.createElement("option"); opt.value = d; opt.innerText = d; selTo.appendChild(opt);
        });
...
function renderDashboard(mode, fromD, toD, targetUPT, targetATV, targetAUR, discFilter = "ALL", catFilter = "ALL") {
    ...
    let totalSales = 0, totalQty = 0, totalSM = 0, totalTargetSales = 0;
    let totalAcc = 0, totalAccSales = 0, totalAccSM = 0;
    let totalBags = 0, totalBagsSales = 0, totalBagsSM = 0;
    let totalApp = 0, totalAppSales = 0, totalAppSM = 0;
    let totalFtw = 0, totalFtwSales = 0, totalFtwSM = 0;
    let totalO2OSales = 0, totalO2OSM = 0, totalO2OQty = 0;
    let articleAgg = {};
    ...
    selectedDates.forEach(d => {
        ...
        for(const art in data.articles) {
            if(!articleAgg[art]) articleAgg[art] = { qty: 0, sales: 0, division: data.articles[art].division };
    ...
    if (catBody) {
        ...
        const CAT_LABELS = { ACC: "ACCESSORIES", BAG: "BAG", APP: "APPAREL", FTW: "FOOTWEAR" };
        ...
        CATS.forEach(cat => {
            ...
            let catTotalQty = 0, catTotalSales = 0;
            types.forEach(t => {
        ...
        if (catBody.innerHTML === "") {
            catBody.innerHTML = '<tr><td colspan="4" style="text-align:center; padding:20px;">BELUM ADA DATA</td></tr>';
        }
    }
...
function toggleDiscGroup(groupId) {
    const rows = document.querySelectorAll(`.disc-sub-row[data-group="${groupId}"]`);
    const header = document.querySelector(`.disc-cat-header[onclick*="${groupId}"]`);
    let isHidden = false;
    rows.forEach(r => {
...
function renderTopArticles(articleAgg) {
    ...
    const topSales = arr.slice().sort((a,b) => b.sales - a.sales).slice(0, 5);
    const topQty = arr.slice().sort((a,b) => b.qty - a.qty).slice(0, 5);
    const topFootwear = arr.slice()
        .filter(a => a.division.includes("FOOTWEAR") || a.division.includes("SHOES") || a.division.includes("SEPATU"))
        .sort((a,b) => b.qty - a.qty)
        .slice(0, 5);

    const renderList = (items, isSales) => {
        if(items.length === 0) return "-";
        let html = '<ol style="margin-left: 20px; font-size: 14px; line-height: 1.5;">';
        items.forEach(it => {
            const val = isSales ? "Rp " + Math.round(it.sales).toLocaleString('id-ID') : Math.round(it.qty).toLocaleString('id-ID') + " pcs";
